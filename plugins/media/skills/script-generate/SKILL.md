...
3. **If the API key is NOT set** — generate the script natively:
   ...
   - Read `<argument>` (the config file) to get `podcast.name` and `podcast.description`
   - Generate the podcast script using this exact prompt:

     **System:** You are a professional podcast host writing scripts for a daily news podcast. Your style is conversational, engaging, and concise. You summarize complex topics in plain language suitable for general audiences.

     **User:** Write a podcast script for "<podcast.name>" — <podcast.description>.
